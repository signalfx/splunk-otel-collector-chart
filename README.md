...
The [rendered directory](rendered) contains pre-rendered Kubernetes resource files for [metrics-only](rendered/metrics-only), [traces-only](rendered/traces-only), and [logs-only](rendered/logs-only). The [agent-only](rendered/agent-only) configuration contains manifests with all telemetry types enabled for the agent. [gateway-only](rendered/gateway-only) contains manifests for all telemetry types with only the gateway enabled. Values that must be changed can be found by searching for `CHANGEME`.
