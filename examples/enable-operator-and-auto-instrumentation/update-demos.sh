...
function update_spring_petclinic_demo {
    ...
    # Remove unwanted port settings
    yq eval -i 'del(.spec.template.spec.containers[].ports[].hostPort) | del(.spec.template.spec.containers[].ports[].protocol)' "$SPRING_PETCLINIC_PATH"
