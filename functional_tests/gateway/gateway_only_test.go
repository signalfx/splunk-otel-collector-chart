...
func sendMetric(t *testing.T) {
	...
	// Send to kind cluster exposed port 99430, http forwarder sends to 9943
	req, err := http.NewRequest(http.MethodPost, "http://"+internal.HostPort("127.0.0.1", 99430)+"/v2/datapoint", bytes.NewBuffer(data))
	require.NoError(t, err)
