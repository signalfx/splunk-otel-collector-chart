...
- `chart`: Bump TargetAllocator to 0.158.0 in helm-charts/splunk-otel-collector/Chart.yaml ([#2560](https://github.com/signalfx/splunk-otel-collector-chart/pull/2560))
- `operator`: Bump java to v2.31.1 in helm-charts/splunk-otel-collector/values.yaml ([#2557](https://github.com/signalfx/splunk-otel-collector-chart/pull/2557))

## [0.160.0] - 2026-09-11

This Splunk OpenTelemetry Collector for Kubernetes release adopts the [Splunk OpenTelemetry Collector v0.160.1](https://github.com/signalfx/splunk-otel-collector/releases/tag/v0.160.1).
...
- `agent`: Drop `server.address` and `server.port` dimensions from Prometheus metrics ([#2288](https://github.com/signalfx/splunk-otel-collector-chart/pull/2288))
  Prometheus metrics (especially coming from Istio) can run into dimension limits.
  To avoid data points being rejected due to exceeding the dimension limit, these
  dimensions will now be dropped when either `autodetect.prometheus` or `autodetect.istio`
...
- `agent`: Add resourcedetection processor to the agent logs/host pipeline ([#2385](https://github.com/signalfx/splunk-otel-collector-chart/pull/2385))
  ...
  `logsCollection.extraFileLogs` entries are defined. On distributions that support
...
- `agent`: Fix failed agent service creation when no ports are exposed. The agent k8s service is now only created when there are ports to expose ([#2166](https://github.com/signalfx/splunk-otel-collector-chart/pull/2166))
