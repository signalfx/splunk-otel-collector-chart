...
if [[ "${K8S_POD_NAME}" == *-0 ]]; then
  ...
  ./yq e '.receivers.receiver_creator.receivers.kubelet_stats.rule = .receivers.receiver_creator.receivers.kubelet_stats.rule + " && labels[\"splunk-otel-eks-fargate-kubeletstats-receiver-node\"] == \"true\""' /conf/relay.yaml >/splunk-messages/config.yaml
...
./yq e -i '.receivers.receiver_creator.receivers.kubelet_stats.rule = .receivers.receiver_creator.receivers.kubelet_stats.rule + " && not ( name contains \"${K8S_NODE_NAME}\" )"' /splunk-messages/config.yaml
